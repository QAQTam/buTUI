...
SplitPane / Toast / Tooltip / Popover / Portal / Dynamic / MultiSelect / Form / Autocomplete / 高频 chunk 合帧 / smooth reveal 已跑通**，`bun test`
778 个用例全绿。
...
<ToastViewport
  ...
  onAction={(toast, action) => handleToastAction(toast, action)}
/>
...
- `dedupeKey` 会合并重复通知并增加 `count`。
- `maxVisible` 超限时自动关闭最旧项。
- hover 时暂停 TTL，移出后继续剩余时间。
- Enter 执行首个 action，Esc / 点击 `×` 关闭。
...
- 不传 `mount` 时默认挂到 app root。
- 原位置只保留零宽 sentinel，不占布局。
- children 保持响应式。
- mount 变化或卸载时自动移除容器。
...
- `component` 可以是函数组件或 intrinsic tag 字符串。
- 切换组件类型时重建目标节点。
- props / children 保持响应式；`undefined` 渲染为空。
- `createDynamic()` 可用于库内部的低开销动态渲染。
...
- 受控 values，不维护第二份选择状态。
- Space / Enter / 点击切换当前项。
- Ctrl+A 全选，Ctrl+Shift+A 清空。
- heading / disabled 只展示，不参与导航或选择。
- 复用 `<List>` / `createSelection()` 的滚动与键盘行为。
...
- 目标节点负责 click / key 事件，Popover 负责定位与内容。
...
- dirty / touched / errors / submitting / submitted 都由 model 提供。
- 支持同步 / 异步 validate 和 async submit。
- `<Form>` 提供 `useForm()` context；Enter 默认提交。
- `<FormField>` 展示 label / required / touched error。
...
- `filterAutocompleteOptions()` 对 label / value / description 做评分过滤。
- ↑↓ / PageUp/PageDown / Home/End 导航，Enter 选择。
- Esc 清空 query；点击结果直接选择。
- disabled 项展示但不会被选中。
