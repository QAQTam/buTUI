...
字节，frame 120fps 写入 143 次 / 14430 字节；smooth reveal 120fps 写入 176
次 / 17653 字节，但可见内容是逐帧连续推进，而不是一次跳块。

**stdout backpressure。** `TerminalSession.write()` 透传 Node 的返回值；返回
`false` 时 renderer 在 `RenderStats.blocked` 上报，runtime 暂停自动绘制，只把
节点标脏。stdout 触发 `drain` 后解除阻塞并合并画最新树。这样 120fps 遇到慢终端
时，写缓冲不会持续增长，也不会用旧帧排队。
