...
- 插件已有 manifest / 配置 / 直接依赖发现 / capability 白名单、运行时审批和
  lease / TTL / revoke；仍无 worker / 跨进程隔离和审批 UI。
...
4. **插件 capability UI / 隔离**：审批 + lease / TTL / revoke 已完成；下一步做
   审批 UI、worker 隔离和跨进程 capability 代理。
