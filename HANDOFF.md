...
> 工作区状态：本文件与 Worker capability proxy 一并提交
> 本轮能力：worker RPC + crash fail-fast + capability proxy
> 当前回归：`716 pass / 0 fail`，79 个测试文件，`tsc --noEmit` 通过
...
- Plugin Worker RPC：`createWorkerRpc` / `serveWorkerRpc` 提供 structured clone
  ...
  Worker 只做崩溃 / 堆隔离，不是 capability sandbox。
...
- 插件已有 manifest / 配置 / 直接依赖发现 / capability 白名单、运行时审批、
  approval queue / dialog、lease / TTL / revoke、Worker RPC、crash fail-fast 和
  capability proxy；仍无 restart / supervision、进程 adapter 与 OS sandbox。
...
4. **插件 worker / 跨进程隔离**：审批队列、dialog、lease / TTL / revoke、
   Worker RPC、crash fail-fast 与 capability proxy 已完成；下一步做 restart /
   supervision、进程 adapter 和 OS sandbox 边界。
