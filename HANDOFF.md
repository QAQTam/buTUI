...
> 工作区状态：本文件与 Popover 组件一并提交
> 本轮能力：interactive overlay controller + layer
> 当前回归：`772 pass / 0 fail`，92 个测试文件，`tsc --noEmit` 通过
...
| `@butui/components` | 编辑器、Input、Textarea、List、Diff、ScrollBar、Slider、SplitPane、Toast、Tooltip、Popover、Portal、Dynamic、MultiSelect、弹窗等 |
...
- Popover：`createPopoverController` / `<Popover>` 支持点击切换、四向定位、
  交互内容与 Esc dismiss。
...
- `CapabilityBroker`：grant / has / active / revoke / TTL expire / revokeAll；
...
- `createToastQueue()`：TTL、dedupeKey + count、maxVisible overflow、
  pause / resume、clear / dispose 和 added / updated / dismissed 事件。
- `<ToastViewport>`：根 `<layer>` 定位，支持四角 placement、message、tone、
  action buttons、Enter 主 action、Esc / 鼠标关闭、hover pause。
- 已有 `tests/toast.test.tsx`。

### 5.14.2 Tooltip

- `createTooltipController()`：延迟 show、延迟 hide、锚点更新、dispose。
- `tooltipPosition()`：top / bottom / left / right 定位与视口夹取。
- `<Tooltip>`：独立根 `<layer>`，目标节点自行绑定 enter / move / leave，
  避免子节点 hit test 导致 leave 不冒泡。
- 已有 `tests/tooltip.test.tsx`。

### 5.14.3 Portal

- `<Portal mount={node}>` 把 children 插入指定 mount；不传时使用 app root。
- 原位置返回零宽 sentinel，不占布局。
...
- `createMultiSelect()`：受控 values、toggle / setSelected / selectAll / clear /
  invert；heading / disabled 不参与选择。
- `<MultiSelect>`：复用 List / Selection；Space / Enter / 点击切换，
  Ctrl+A 全选，Ctrl+Shift+A 清空。
- 已有 `tests/multi-select.test.tsx`。
...
- Process resource limits：`spawnProcessRpc` 映射 Bun timeout / maxBuffer /
  ...
  外部 memory.max / cpu.max / pids.max。
- Cgroup lifecycle：`createCgroupManager` 支持 v2 detect、create、lease release、
  stale recover、自有 prefix 过滤与 EBUSY 重试；无权限时明确 unsupported。
- Audit persistence：memory / NDJSON file log 支持 type / plugin / 时间查询；
  ...
  retention / summary，`withAuditSinks` + HTTP NDJSON sink 支持批量远端汇聚、
...
- 插件已有 manifest / 配置 / 直接依赖发现 / capability 白名单、运行时审批、
  ...
  explicit ack / source ordering / pending recovery / cleanup；仍无自动 cgroup 委派和
  OS 权限 sandbox。
